  ...
  balance: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onDragEnd: function onDragEnd(event) {
        ...
        var leftDistance = cc.pDistance(target, leftPos);
        var rightDistance = cc.pDistance(target, rightPos);
        var posType = "";
        ...
        this.node.on("drag-end", function(event) {
          ...
          _this.fires.forEach(function(fire) {
            if (fire.active && fire.uuid !== target.uuid) {
              var distance = cc.pDistance(target.position, fire.position);
    ...
    cc.Class({
      ...
      onTouchend: function onTouchend(event) {
        ...
        if (null !== this.stickTarget) {
          ...
          var distance = cc.pDistance(targetPos, pos);
    ...
    cc.Class({
      ...
      getData: function getData() {
        ...
        if (IN_FB) FBInstant.player.getDataAsync([ "keyCount", "puzzleStep", "rewardShared" ]).then(function(data) {
          ...
          var keyCount = Utils.getData("keyCount");
  ...
  "puzzle-btn-list": [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onSwiped: function onSwiped(event) {
        ...
        var isEdge = false;
        "left" === event.detail.direction ? index < this.navArr.length - 1 ? index += 1 : isEdge = true : "right" === event.detail.direction && index > 0 && (index > 0 ? index -= 1 : isEdge = true);
